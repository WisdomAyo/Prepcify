...
export default function StudyPlanTodayPage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="space-y-3">
        {tasks.map((t) => (
          <button
            ...
            onClick={() => toggle(t.id)}
            className={cn(
              "flex w-full items-center gap-4 rounded-2xl border bg-card p-5 text-left transition-all",
              t.done
      ...
      <div className="mt-8 flex items-center gap-4 rounded-2xl bg-foreground p-5 text-background">
        <Sparkles className="h-5 w-5 text-accent" />
        <p className="flex-1 text-sm">
          prepcify AI noticed you&apos;re stronger on Algebra than Geometry. Want a
          custom drill?
        </p>
