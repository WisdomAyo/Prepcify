  ...
  return (
    <div className="max-w-4xl">
      <PageHeader
        ...
        eyebrow="Tool"
        title="Math Solver"
        description="Type any equation. We'll factor, expand, integrate, differentiate."
      />
      ...
      <div className="rounded-3xl border border-border bg-card p-6">
        ...
        <div className="mt-3 flex gap-2">
          ...
          <button className="rounded-2xl bg-accent px-6 text-sm font-semibold text-accent-foreground">
            Solve
          </button>
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-xs">
          {["x^2 + 5x = 6", "d/dx (3x^2 + 2x)", "∫(2x + 1)dx", "sin(x) = 0.5"].map(
            (s) => (
              <button
                key={s}
                onClick={() => setEq(s)}
                className="rounded-full bg-secondary px-3 py-1 font-mono"
              >
                {s}
              </button>
            ),
