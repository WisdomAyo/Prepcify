...
export default function Pricing() {
  return (
    <div className="max-w-6xl">
      <PageHeader eyebrow="Plans" title="Pick the plan that fits how you study" description="Cancel anytime. Refunds within 7 days." />
      <div className="grid gap-5 lg:grid-cols-3">
        {plans.map((p) => (
          <div
            key={p.id}
            className={`relative rounded-3xl border p-7 ${p.accent ? "border-accent bg-foreground text-background shadow-card" : "border-border bg-card"}`}
          >
            {p.popular && (
            ...
            <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${p.accent ? "bg-accent text-accent-foreground" : "bg-secondary"}`}>
              <p.icon className="h-5 w-5" />
            </div>
            <h3 className="mt-5 font-display text-2xl font-extrabold">{p.name}</h3>
            <div className="mt-2 flex items-baseline gap-1">
              <p className="font-display text-4xl font-extrabold">{p.price}</p>
              <p className={`text-sm ${p.accent ? "text-background/60" : "text-muted-foreground"}`}>{p.cycle}</p>
            </div>
            <ul className="mt-6 space-y-2.5 text-sm">
              {p.features.map((f) => (
                <li key={f} className="flex items-start gap-2">
                  <Check className={`h-4 w-4 shrink-0 mt-0.5 ${p.accent ? "text-accent" : "text-success"}`} />
                  <span>{f}</span>
                </li>
              ))}
            </ul>
            <Link
              to={p.id === "free" ? "/app" : "/app/checkout"}
