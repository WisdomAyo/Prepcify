...
export default function QuizResults() {
  return (
    <div className="max-w-5xl">
      <PageHeader eyebrow="Results" title="Nice run, you scored 17/20" description="Better than 73% of students who took this quiz today." />

      <div className="grid gap-5 md:grid-cols-3 mb-6">
        <div className="rounded-3xl border border-border bg-card p-6 text-center">
          <RingProgress value={85}><span className="font-display text-2xl font-extrabold">85%</span></RingProgress>
          <p className="mt-4 text-sm font-semibold">Score</p>
        </div>
        <div className="rounded-3xl bg-foreground text-background p-6">
          <Trophy className="h-5 w-5 text-accent" />
          <p className="mt-3 font-display text-3xl font-extrabold">+340 XP</p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <Link to="/app/quiz/review" className="rounded-full bg-secondary px-5 py-2.5 text-sm font-semibold">
          Review answers
        </Link>
        <Link to="/app/quiz/setup" className="rounded-full bg-foreground px-5 py-2.5 text-sm font-semibold text-background">
          New quiz
        </Link>
        <Link to="/app" className="rounded-full bg-accent px-5 py-2.5 text-sm font-semibold text-accent-foreground">
          Back to dashboard
        </Link>
      </div>
    </div>
