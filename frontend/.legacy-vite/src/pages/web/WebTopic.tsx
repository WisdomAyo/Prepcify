...
import { ArrowLeft, Check, Lock, Play, Sparkles, Clock, FileText, Calculator } from "lucide-react";
...
export default function WebTopic() {
  const { subjectId } = useParams();
  return (
    <div className="space-y-8 max-w-6xl">
      <Link to="/app/subjects" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <section className="lg:col-span-2 rounded-3xl bg-foreground p-8 text-background">
          <p className="text-xs uppercase tracking-wider text-background/60">{(subjectId ?? "math").toUpperCase()} · JAMB · Senior Secondary</p>
          <h1 className="mt-3 font-display text-4xl font-extrabold tracking-tight">Mathematics</h1>
          <p className="mt-2 text-background/70 max-w-lg">Algebra, geometry, calculus and statistics — structured the way examiners ask.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Button asChild size="lg" variant="accent"><Link to="/app/quiz"><Play /> Continue learning</Link></Button>
            <Button asChild size="lg" variant="outline" className="bg-transparent text-background border-background/25 hover:bg-background/10 hover:text-background">
              <Link to="/app/exam"><FileText /> Past papers</Link>
            </Button>
          </div>
        </section>

        <section className="rounded-3xl border border-border bg-card p-6 shadow-soft flex items-center gap-5">
          <RingProgress value={68} size={108} stroke={11} indicatorClassName="stroke-foreground">
          ...
          <div>
            ...
            <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><Calculator className="h-3.5 w-3.5" /> 24 topics</span>
              <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> 18h total</span>
            </div>
          </div>
        </section>
      </div>
              ...
              <li key={t.id}>
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "border-foreground shadow-card" : "border-border hover:shadow-soft",
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{t.lessons} lessons · {t.time}</p>
                    {active && <BarProgress value={t.progress!} className="mt-2 max-w-sm" height={4} />}
                  </div>
                  {active && (
                    <div className="hidden sm:flex items-center gap-2 rounded-full bg-foreground px-4 py-2 text-background text-xs font-semibold">
      ...
      <section className="flex items-center gap-4 rounded-3xl bg-foreground p-6 text-background">
        ...
        <div className="flex-1">
          <p className="font-display text-lg font-bold leading-tight">Need a different angle?</p>
          <p className="text-sm text-background/70">prepcify AI can re-explain anything in plain English.</p>
        </div>
        <Button asChild variant="accent"><Link to="/app/tutor">Open AI tutor</Link></Button>
      </section>
    </div>
  );
}
