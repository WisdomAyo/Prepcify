  ...
  return (
    <div className="max-w-7xl">
      <PageHeader
        ...
        actions={
          <>
            <Link to="/admin/questions/upload" className="rounded-full bg-secondary px-5 py-2.5 text-sm font-semibold flex items-center gap-2">
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <label className="flex flex-1 max-w-md items-center gap-2 rounded-2xl bg-card border border-border px-4 py-2.5">
          <Search className="h-4 w-4 text-muted-foreground" />
          <input placeholder="Search by ID, keyword, topic…" className="flex-1 bg-transparent text-sm outline-none" />
        </label>
        {["All exams", "All subjects", "All years"].map((f) => (
          <button key={f} className="rounded-2xl bg-secondary px-4 py-2.5 text-xs font-semibold">{f} ▾</button>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-secondary/40 border-b border-border">
            <tr className="text-[11px] uppercase tracking-wider text-muted-foreground">
              ...
              <th className="text-left px-5 py-3 font-semibold">Exam · Year</th>
          ...
          <tbody>
            {questions.map((q) => (
              <tr key={q.id} className="border-b border-border last:border-0 hover:bg-secondary/30">
                ...
                <td className="px-5 py-3">{q.exam} · {q.year}</td>
                <td className="px-5 py-3">{q.subject}</td>
                <td className="px-5 py-3">
                  <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${diffColor[q.difficulty]}`}>{q.difficulty}</span>
                </td>
                <td className="px-5 py-3">
                  <span className={`text-xs ${q.status === "Published" ? "text-success" : "text-muted-foreground"}`}>{q.status}</span>
                </td>
                <td className="px-5 py-3 text-right">
                  <div className="flex justify-end gap-2">
                    <button className="p-1.5 rounded-md hover:bg-secondary"><Edit className="h-3.5 w-3.5" /></button>
