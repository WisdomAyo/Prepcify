...
export default function AdminUsers() {
  return (
    <div className="max-w-7xl">
      <PageHeader eyebrow="Admin" title="Users" description="12,480 active users · 1,840 paying" />

      <label className="mb-4 flex items-center gap-2 rounded-2xl bg-card border border-border px-4 py-2.5 max-w-md">
        <Search className="h-4 w-4 text-muted-foreground" />
        <input placeholder="Search by name, email, ID…" className="flex-1 bg-transparent text-sm outline-none" />
      </label>
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-border last:border-0 hover:bg-secondary/30">
                ...
                <td className="px-5 py-3">
                  <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${planColor[u.plan]}`}>{u.plan}</span>
                </td>
                <td className="px-5 py-3 font-mono text-xs">{u.xp.toLocaleString()}</td>
                <td className="px-5 py-3 text-xs text-muted-foreground">{u.joined}</td>
                <td className="px-5 py-3">
                  <span className={`text-xs ${u.status === "Active" ? "text-success" : "text-destructive"}`}>{u.status}</span>
                </td>
                <td className="px-5 py-3 text-right">
                  <button className="p-1.5 rounded-md hover:bg-secondary"><MoreVertical className="h-3.5 w-3.5" /></button>
                </td>
              </tr>
            ))}
          </tbody>
